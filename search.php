...
$posts_per_page=$user["searchprods"]?$user["searchprods"]:10;
...
if($what) {
  ...
  if($type=="prod") {
    ...
    $query="SELECT bbs_topics.id,bbs_topics.topic,bbs_topics.lastpost,bbs_posts.post FROM bbs_topics LEFT JOIN bbs_posts ON bbs_topics.id=bbs_posts.topic WHERE bbs_topics.topic LIKE '".$subquery."' OR bbs_posts.post LIKE '".$subquery."' GROUP BY bbs_topics.id ORDER BY bbs_topics.lastpost DESC";
    $query.=" LIMIT ".(($page-1)*$posts_per_page).",".$posts_per_page;
  }
  $result=mysql_query($query);
    ...
    <tr>
      ...
      <table cellspacing="0" cellpadding="0">
       <tr>
        ...
        <td><input type="text" name="what" size="20" value="<? print(htmlentities(stripslashes($what))); ?>"></td>
...
<? elseif($type=="bbs"): ?>
<table bgcolor="#000000" cellspacing="1" cellpadding="0" border="0" width="600">
 <tr>
  <td>
   <table bgcolor="#000000" cellspacing="1" cellpadding="2" border="0">
    <tr bgcolor="#224488">
   ...
   <tr bgcolor="#446688">
   <td valign="top">
    <a href="topic.php?which=<?=$results[$i]["id"]?>"><?
    $s = htmlcleanonerow($results[$i]["topic"]);
/*
    foreach($a as $v) {
...
*/
   ...
   <?
   $a = explode(" ",$what);
   //$s = strip_tags(parse_message($results[$i]["post"]));
   $s = strip_tags(parse_message($results[$i]["post"]));
   $s = preg_replace("/(\s+)/"," ",$s);
   $s = htmlspecialchars(mb_strcut($s,max(0,stripos($s,$a[0])-50),100,"utf-8"));
   foreach ($a as $v2) {
