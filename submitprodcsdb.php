...
if($csdbflag)
{
  ...
  $query = "SELECT csdb FROM prods WHERE id=".$which;
  $result = mysql_query($query);
  if(mysql_result($result,0)>0)
    $errormessage[] = "csdb for this prod already exists (\/) O_o (\/)";
...
if($submitok){
  $query ="update prods set csdb=".((int)$csdbflag)." where id=".$which;
  mysql_query($query);
}

?>
<br>
<form action="submitprodcsdb.php" method="post" enctype="multipart/form-data">
<INPUT TYPE="hidden" name="MAX_FILE_SIZE" value="65536">
<table bgcolor="#000000" cellspacing="1" cellpadding="0"><tr><td>
<table bgcolor="#000000" cellspacing="1" cellpadding="2">
<? if($submitok): ?>
 <tr><th bgcolor="#224488">csdb id for prod submited</th></tr>
 <tr>
 ...
 <tr>
  <td bgcolor="#446688">
   <table>
	...
	<? else:
	// build the form
	$query="SELECT prods.id,prods.name FROM prods LEFT JOIN prods_platforms on prods_platforms.prod=prods.id where prods.csdb=0 and (prods_platforms.platform=76 or prods_platforms.platform=133) ORDER BY prods.name";
	$result=mysql_query($query);
	while($tmp=mysql_fetch_array($result)) {
		$prods[]=$tmp;
	}
	?>
	 <td>select the prod you want<br>to add csdb id for:</td>
     <td valign="bottom">
      <select name="which">
