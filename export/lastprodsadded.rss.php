...
foreach($prods as $p)
{
	...
	$output.= $t.$t.$t.'<link>http://www.pouet.net/prod.php?which='.$p['id'].'</link>'.$nl;
	// enclosure tag
	unset($screenshot);
	...
	if($screenshot)
	{
		...
		$output.= $t.$t.$t.'<enclosure url="http://www.pouet.net/screenshots/'.$screenshot.'" length="'.$screenshot_size.'" type="'.$screenshot_type.'" />'.$nl;
	}
