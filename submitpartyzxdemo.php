...
if($zxdemoflag)
{
  ...
  $query = "SELECT zxdemo FROM partylinks WHERE party=".$which." and year=".$when;
  $result = mysql_query($query);
  if(mysql_result($result,0)>0)
    $errormessage[] = "zxdemo for this event already exists! (\/) O_o (\/)";
...
if($submitok){
  ...
  if($thisid)
  {
  	$query ="UPDATE partylinks SET zxdemo='".$zxdemoflag."' where id=".$thisid;
  	mysql_query($query);
  }
  else
  {
	$query ="INSERT INTO partylinks SET party=".$which.", year=".$when.", zxdemo='".$zxdemoflag."'";
  	mysql_query($query);
  }
}
...
?>
<br>
<form action="submitpartyzxdemo.php" method="post" enctype="multipart/form-data">
<INPUT TYPE="hidden" name="MAX_FILE_SIZE" value="65536">
<table bgcolor="#000000" cellspacing="1" cellpadding="0"><tr><td>
<table bgcolor="#000000" cellspacing="1" cellpadding="2">
<? if($submitok): ?>
 <tr><th bgcolor="#224488">zxdemo id for event submited</th></tr>
 <tr>
 ...
 <tr>
  <td bgcolor="#446688">
   <table>
	...
	?>
	 <td>select the event you want<br>to add zxdemo id for:</td>
     <td valign="bottom">
      <select name="which">
